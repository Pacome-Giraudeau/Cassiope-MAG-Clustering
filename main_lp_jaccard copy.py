    ...
    with open(output_path, "w") as f:
        ...
        for i in range(n):
            ...
            for j in range(i, n):  # Évite les doublons (distance symétrique)
                ...
                distance_coverage = jaccard(coverage_c1, coverage_c2)
                distance_kmere = jaccard(kmere_c1, kmere_c2)
...
def on_model(model, rules_name):
    ...
    # Sauvegarde
    clusters_output_file = os.path.join(folder_data_working, f"jaccard_OC_{nb_contigs_to_classify}_clusters_{rules_name}.txt")

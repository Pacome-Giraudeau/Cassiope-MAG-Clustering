...
def on_model(model):
    ...
    with open(os.path.join(folder_data_working, f"{nb_contigs_to_classify}_clusters.txt"), "w") as f:
        for atom in atoms:
...
def init_clingo():
    ...
    file_rules = add_clingo_rules()

    file_output = os.path.join(folder, "output.lp")
    ...
    print("Création du fichier progromme ok !")
